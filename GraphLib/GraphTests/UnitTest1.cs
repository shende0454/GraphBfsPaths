...
namespace GraphTests
{
    ...
    {
        //// Create a basic graph class that implements IGraph
        [TestMethod]
        ...
        [TestMethod]
        public void createMultinodeGraph()
        {
            BasicGraph myGraph = new BasicGraph();
            myGraph.AddEdge(0, 1);
            myGraph.AddEdge(0, 3);
            myGraph.AddEdge(0, 4);
            List<int> results = new List<int>() { 1, 3, 4 };
            foreach (var vertex in myGraph.GetAdjacentVertices(0))
            {
                Assert.IsTrue(results.Contains(vertex));
        ...
        // // Copy GraphLib/GraphTests/oneEntry.txt to cs242\lab5 in your
        ...
        public void readOneEntrySymbolGraph()
        {
            ...
            string filename = path + @"\cs242\lab5\oneEntry.txt";
            SymbolGraph myGraph = new SymbolGraph(filename, '/');
            int counter = 0;
        ...
        {
            string path = System.Environment.GetEnvironmentVariable("HOMEPATH");
            string filename = path + @"\cs242\lab5\tenMovies.txt";
            SymbolGraph myGraph = new SymbolGraph(filename, '/');
            int counter = 0;
            foreach (int vertex in
        ...
        {
            string path = System.Environment.GetEnvironmentVariable("HOMEPATH");
            string filename = path + @"\cs242\lab5\movies.txt";
            SymbolGraph myGraph = new SymbolGraph(filename, '/');
            int counter = 0;
            foreach (int vertex in
                myGraph.GetAdjacentVertices(myGraph.KeyToVertexId["'Breaker' Morant (1980)"]))
